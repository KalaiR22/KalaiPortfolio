...
const WorkExperience = () => {
  return (
    <div className=" px-4  sm:px-10 md:px-10 xl:px-14 2xl:px-20 flex flex-col py-16 ">
      ...
      <div className="flex flex-col gap-36 mx-auto md:m-0  mg:px-1 lg:px-5 xl:px-10 py-32">
        {workexperience.map((work) => (
          <div className="flex flex-col md:flex-row gap-8  md:justify-between" key={work.id}>
            <div className="flex justify-center gap-2 sm:gap-4">
              <div className="  pt-[1rem] md:pt-[2rem] lg:pt-[2.2rem] xl:pt-[3.4rem]">
                <img src={ministar} alt="star" className='h-[1.5rem] w-[1.5rem] sm:h-[2rem] sm:w-[2rem]' />
              </div>
              <div className="flex flex-col justify-center  leading-[3.5rem] lg:leading-[4.8rem]">
                <div className="titlefour">{work.company}</div>
                <div className="titlefive">{work.role}</div>
                <div className="titlefive">{work.part}</div>
              </div>
            </div>
            <div className="flex flex-col sm:w-[350px] md:w-[330px] lg:w-[460px] xl:w-[565px]  gap-8 lg:gap-10">
